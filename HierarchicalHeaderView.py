...
"""
Внешний модуль многострочного заголовка таблицы
"""
...
class private_data:
        ...
        def parentIndexes(self, index: QModelIndex)->QList:
            indexes = QList()
            while index.isValid():
        ...
        def searchLeafs(self, currentIndex: QModelIndex)->QList:
            res = QList()
            if currentIndex.isValid():
                childCount=currentIndex.model().columnCount(currentIndex)
                if childCount:
        ...
        def leafs(self, searchedIndex: QModelIndex)->QList:
            leafs = QList()
        ...
        def currentCellWidth(self, searchedIndex: QModelIndex, leafIndex: QModelIndex, sectionIndex: int, hv: QHeaderView)->int:
            leafsList = QList(self.leafs(searchedIndex))
            if leafsList.empty():
        ...
        def paintHorizontalSection(self, painter: QPainter, sectionRect: QRect,
            ...
            top = sectionRect.y()
            indexes = QList(self.parentIndexes(leafIndex))
...
class HierarchicalHeaderView(QHeaderView):
    """
    Многострочный заголовок таблицы
    """

    options = {"highlightSections": True, "clickable": True}

    def __init__(self, orientation: Qt.Orientation, parent: QtWidgets.QWidget):
        ...
        elif isinstance(parent, QtWidgets.QTreeView):
            getattr(parent, "setHeader")(self)
    ...
    def on_sectionResized(self, logicalIndex: int):
        ...
        leafIndex = QModelIndex(self._pd.leafIndex(logicalIndex))
        if leafIndex.isValid():
            leafsList = QList(self._pd.leafs(self._pd.findRootIndex(leafIndex)))
...
if __name__=="__main__":
    ...
    QtWidgets.QVBoxLayout(form).addWidget(view)
    form.show()
    ...
    view.horizontalHeader().setSectionsMovable(False) #reorder DataFrame columns manually
